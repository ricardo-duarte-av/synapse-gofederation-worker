...
// The conditional upsert is the subtle part: a shorter backoff must not
// overwrite a longer one, or a dead server gets hammered by whichever request
// happened to fail last.
func TestLiveWriterRetryTimingsConditions(t *testing.T) {
	w := liveWriter(t)
	ctx := context.Background()
	const dest = "retry-test.example"
	_, _ = w.pool.Exec(ctx, `DELETE FROM destinations WHERE destination = $1`, dest)

	now := time.Now().UnixMilli()
	if err := w.SetDestinationRetryTimings(ctx, dest, now, now, 60_000); err != nil {
		t.Fatal(err)
	}
	// A shorter backoff at the same instant must not win.
	if err := w.SetDestinationRetryTimings(ctx, dest, now, now, 1_000); err != nil {
		t.Fatal(err)
	}

	var interval int64
	if err := w.pool.QueryRow(ctx,
		`SELECT retry_interval FROM destinations WHERE destination = $1`, dest).Scan(&interval); err != nil {
		t.Fatal(err)
	}
	if interval != 60_000 {
		t.Errorf("retry_interval = %d, want the longer backoff to stand", interval)
	}

	// A success clears it, even against a standing backoff.
	if err := w.ClearDestinationRetryTimings(ctx, dest); err != nil {
		t.Fatal(err)
	}
	var failure *int64
	if err := w.pool.QueryRow(ctx,
		`SELECT retry_interval, failure_ts FROM destinations WHERE destination = $1`,
		dest).Scan(&interval, &failure); err != nil {
		t.Fatal(err)
	}
	if interval != 0 || failure != nil {
		t.Errorf("after success: interval=%d failure_ts=%v, want 0 and NULL", interval, failure)
	}
}
